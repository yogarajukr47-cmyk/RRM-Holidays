...
const WHATSAPP_URL = `https://wa.me/919108597154?text=${encodeURIComponent("Hi! I'm looking for destinations in Andhra Pradesh with RRM Holidays.")}`;

export default function AndhraNotFound() {
  return (
    <div className="min-h-screen bg-neutral-950 flex items-center justify-center px-4 py-20">
      <div className="fixed inset-0 pointer-events-none overflow-hidden">
        <div className="absolute top-1/4 left-1/3 w-72 h-72 bg-rose-500/5 rounded-full blur-3xl" />
        <div className="absolute bottom-1/4 right-1/3 w-60 h-60 bg-amber-500/5 rounded-full blur-3xl" />
      </div>
      ...
      <div className="relative z-10 max-w-md w-full text-center">
        <div className="bg-neutral-900/50 backdrop-blur-xl border border-white/5 rounded-2xl p-8 md:p-10">
          ...
          <span className="inline-block px-3 py-1 rounded-full bg-rose-500/10 border border-rose-500/20 text-rose-400 text-xs font-medium mb-4">
            Andhra Pradesh
          </span>
          ...
          <p className="text-neutral-400 text-sm leading-relaxed mb-8">
            This Andhra Pradesh destination doesn&apos;t exist or may have been moved. Explore our popular Andhra destinations!
          </p>

          <div className="flex flex-wrap justify-center gap-2 mb-8">
            {['Tirupati', 'Visakhapatnam', 'Araku', 'Vijayawada', 'Rajahmundry'].map((place) => (
              <Link
                key={place}
                href={`/destinations/andhra/${place.toLowerCase()}`}
