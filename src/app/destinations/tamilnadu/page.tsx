  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a]">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 glass py-2">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 flex items-center justify-between">
          ...
          <div className="hidden lg:flex items-center gap-6">
            ...
            <a href={`https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent('Hi RRM Holidays! I want to plan a trip to Tamil Nadu. Please share details.')}`} target="_blank" className="px-5 py-2.5 rounded-full bg-gradient-to-r from-amber-500 to-amber-600 text-black text-sm font-semibold">Plan My Trip</a>
          </div>
      ...
      <section className="relative min-h-[50vh] flex items-center justify-center overflow-hidden">
        ...
        <div className="absolute top-1/4 right-1/4 w-72 h-72 bg-purple-500/5 rounded-full blur-3xl animate-float" />
        <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 text-center pt-24 pb-16">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-white/10 bg-white/5 text-xs font-semibold text-stone-300 mb-6"><Sparkles size={14} className="text-purple-400" /> 22+ Places to Explore</div>
          <h1 className="text-4xl sm:text-5xl md:text-7xl font-extrabold leading-tight tracking-tight mb-4">Discover <span className="text-gradient-warm">Tamil Nadu</span></h1>
          <p className="text-stone-400 text-lg md:text-xl max-w-2xl mx-auto mb-6 leading-relaxed">Ancient temples, hill stations, French colonies, and pristine beaches — experience the cultural heart of South India.</p>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="glass rounded-2xl p-4 md:p-6">
            <div className="flex flex-col md:flex-row gap-4 items-center">
              <div className="relative flex-1 w-full"><Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-stone-500" /><input type="text" placeholder="Search places... (e.g., Chennai, Madurai, Ooty)" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="search-input w-full pl-9 pr-3 py-3 rounded-xl text-sm text-stone-200" /></div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredPlaces.slice(0, visiblePlaces).map((place) => (
              <div key={place.name} className="reveal group rounded-2xl overflow-hidden bg-neutral-900/80 border border-white/5 hover:border-amber-500/20 transition-all hover-lift card-shine">
                ...
                <div className="relative aspect-[16/10] overflow-hidden bg-gradient-to-br from-purple-900/20 to-neutral-900">
