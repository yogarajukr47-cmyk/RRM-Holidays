...
const PLACES = KERALA.places;

function getWhatsAppLink(placeName: string) {
  const msg = `Hi RRM Holidays! I'm interested in visiting ${placeName}, Kerala. Please share travel packages, vehicle options, and pricing details.`;
  ...
  return (
    ...
    <div className="min-h-screen bg-[#0a0a0a]">
        ...
        <div className="fixed inset-0 z-[60]">
          ...
          <div className="absolute right-0 top-0 bottom-0 w-72 bg-neutral-900/95 backdrop-blur-xl border-l border-white/5 p-6 flex flex-col">
            ...
            <div className="flex flex-col gap-1">
              ...
              <Link href="#places" onClick={() => setMobileMenuOpen(false)} className="py-3 px-4 rounded-lg text-stone-300 hover:text-white hover:bg-white/5 text-sm font-medium">📍 Kerala Places</Link>
      ...
      <section className="relative min-h-[50vh] flex items-center justify-center overflow-hidden">
        <Image src="/states/kerala-cover.jpg" alt="Kerala" fill className="object-cover" priority sizes="100vw" />
        <div className="absolute inset-0 bg-gradient-to-b from-black/50 via-black/60 to-[#0a0a0a]" />
        <div className="absolute top-1/4 left-1/4 w-72 h-72 bg-cyan-500/5 rounded-full blur-3xl animate-float" />
            ...
            {filteredPlaces.map((place) => (
              <div key={place.name} className="reveal group rounded-2xl overflow-hidden bg-neutral-900/80 border border-white/5 hover:border-amber-500/20 transition-all hover-lift card-shine">
                ...
                <div className="relative aspect-[16/10] overflow-hidden">
                  <Image src={place.img || `/states/kerala-cover.jpg`} alt={place.name} fill className="object-cover" sizes="(max-width:640px) 100vw, (max-width:1024px) 50vw, 33vw" />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/20 to-transparent" />
                  <span className="absolute top-3 left-3 px-3 py-1 rounded-full text-[10px] font-bold text-cyan-400 bg-cyan-500/10 border border-cyan-500/20 uppercase tracking-wider">{place.category}</span>
                  <div className="absolute bottom-3 left-4 right-4"><h3 className="text-xl font-bold text-white">{place.name}</h3><p className="text-xs text-stone-300 flex items-center gap-1 mt-1"><MapPin size={12} /> Kerala, India</p></div>
                </div>
                </Link>
                <div className="p-5">
                  ...
                  <div className="flex items-center justify-between">
                    <Link href={`/destinations/kerala/${place.slug}`} className="text-xs font-semibold text-cyan-400 group-hover:text-cyan-300 transition-colors">View Details →</Link>
      ...
      <section className="py-20">
        <div className="max-w-4xl mx-auto px-4 sm:px-6">
          <div className="glass rounded-2xl p-8 md:p-12 text-center">
            <h2 className="text-3xl md:text-4xl font-extrabold mb-4">Plan Your <span className="text-gradient-warm">Kerala</span> Trip</h2>
            <p className="text-stone-400 max-w-xl mx-auto mb-8 leading-relaxed">From backwater houseboats to hill station retreats, we create unforgettable Kerala experiences with the best vehicles and local expertise.</p>
            <div className="flex flex-wrap justify-center gap-4">
              <a href={`https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent('Hi RRM Holidays! I want to book a Kerala trip. Please share vehicle options and pricing.')}`} target="_blank" className="inline-flex items-center gap-2 px-8 py-3.5 rounded-full bg-green-600 text-white font-semibold hover:bg-green-500 transition-all"><MessageCircle size={18} /> WhatsApp Us</a>
      ...
      <a href={`https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent('Hi RRM Holidays! I need help planning my Kerala trip.')}`} target="_blank" rel="noopener noreferrer" className="fixed bottom-6 right-6 z-50 w-14 h-14 rounded-full bg-green-500 flex items-center justify-center shadow-2xl hover:bg-green-400 transition-all whatsapp-float"><MessageCircle size={26} className="text-white" /></a>
